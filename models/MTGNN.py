...
class GraphConstructor(nn.Module):
    ...
    def forward(
        ...
        if FE is None:
            ...
            nodevec1 = FE[idx, :]
            nodevec1 = torch.mean(nodevec1, axis=-1)
            nodevec2 = nodevec1
...
class MTGNN(nn.Module):
    ...
    def __init__(
        ...
        # self._classification_layer = nn.Linear(out_dim*in_dim*num_nodes, 2)
        self._classification_layer = nn.Linear(out_dim, 2)
        self._reset_parameters()
    ...
    def forward(
        ...
        """
        seq_len = X_in.size(3)
        FE = X_in.squeeze(1).permute(1, 2, 0)
        assert (
        ...
        X = torch.mean(X.squeeze(-1), axis=-1)
        print(X.shape)
        X = self._classification_layer(X)
        return X